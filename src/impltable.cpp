...
void ImplTable::sort()
{
	...
	for (int lit = 1; lit <= 2 * n; lit++)
		firstBin[lit] = firstBin[lit - 1] + count[lit - 1];
	...
	for (const auto &impl : impls)
		byImplied[start[impl.lit]++] = impl;
	std::swap(byImplied, impls);

	for (int lit = 0; lit < 2 * n; lit++)
	{
		FP_ASSERT(start[lit] == firstBin[lit + 1]);
	}
	sorted = true;
	...
	for (int var = 1; var <= 2 * n; var++)
		firstImplied[var] = firstImplied[var - 1] + count[var - 1];
	start = firstImplied;
	for (const auto &impl : impls)
		byImplied[start[impl.implied]++] = impl;

	for (int var = 0; var < 2 * n; var++)
	{
		FP_ASSERT(start[var] == firstImplied[var + 1]);
	}
	byImpliedValid = true;
	...
	for (int var = 0; var < 2 * n; var++)
	{
		...
		{
			FP_ASSERT((mult * byImplied[i - 1].bound) <= (mult * byImplied[i].bound));
		}
	}
