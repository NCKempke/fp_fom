...
CliqueCover greedyCliqueCover(const CliqueTable &table, const std::vector<int> vars, bool eqOnly)
{
    ...
    for (int cl : cliques)
    {
        ...
        FP_ASSERT(0 <= lastAdded && lastAdded < cc.nCliques());
        for (int lit : cc.getClique(lastAdded))
        {
            const auto [var, isPos] = varFromLit(lit, n);
            FP_ASSERT(!covered[var]);
            covered[var] = true;
        }
    }
    ...
    for (int cl : cliques)
    {
        ...
        {
            const auto [var, isPos] = varFromLit(lit, n);
            if (var2clique[var] == cl)
                projectedClique.push_back(lit);
        }
        FP_ASSERT(projectedClique.size() <= table.getClique(cl).size());
        FP_ASSERT(projectedClique.size() == cliqueCount[cl]);
