 ...
 * @date 2011-2013
 *
 * Copyright 2011-2013 Domenico Salvagnin
 */
#pragma once
...
namespace implementation
{
	...
	template <typename T>
	inline const char *quote() { return ""; }

	template <>
	inline const char *quote<const std::string>() { return "\'"; }

	template <>
	inline const char *quote<std::string>() { return "\'"; }

	template <>
	inline const char *quote<const char *>() { return "\'"; }

	template <>
	inline const char *quote<char *>() { return "\'"; }
					 ...
					 std::streamsize precision = 3, size_t howmany = 5, bool nl = false)
{
	...
	for (size_t i = n - howmany; i < n - 1; i++)
		out << container[i] << ", ";
	out << container[n - 1] << "]";
	if (nl)
		out << std::endl;
...
std::vector<T> split(const std::string &str, const std::string &delim)
{
	...
	while (end < str.size())
	{
		...
		if (end - start != 0)
			ret.push_back(from_string<T>(std::string(str, start, end - start)));
	}
	return ret;
}
