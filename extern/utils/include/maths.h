 ...
 * @date 2008
 *
 * Copyright 2008 Domenico Salvagnin
 */
#pragma once
...
public:
	...
	// Prefix increment
	SparseVectorIteratorType &operator++()
	{
		idx_ptr++;
		coef_ptr++;
		return *this;
	...
	// Postfix increment
	SparseVectorIteratorType operator++(int)
	{
		SparseVectorIteratorType tmp = *this;
		++(*this);
		return tmp;
	}
	// Operators
...
public:
	...
	bool operator==(const SparseVectorViewType &other) const
	{
		if (size() != other.size())
			return false;
		if (!std::equal(idx(), idx() + size(), other.idx()))
			return false;
		if (!std::equal(coef(), coef() + size(), other.coef()))
			return false;
		return true;
	}
	bool operator!=(const SparseVectorViewType &other) const
...
public:
	...
	{
		FP_ASSERT(indices.size() == coefs.size());
	}
	...
	{
		clear();
		for (IdxType i = 0; i < (IdxType)in.size(); ++i)
	...
	{
		if (reset)
			std::fill(out.begin(), out.end(), CoefType(0));
		size_type cnt = size();
		for (size_type i = 0; i < cnt; ++i)
			out[indices[i]] = coefs[i];
	}
...
public:
	...
	{
		if ((k != other.k) || (U != other.U) || (nnz != other.nnz))
			return false;
		for (int i = 0; i < U; i++)
		{
			if ((*this)[i] != other[i])
