 ...
 * @date 2013
 *
 * Copyright 2013 Domenico Salvagnin
 */
...
void Path::mkdir(bool recursive)
{
	...
	else
	{
		...
		while (true)
		{
			...
			if (res && (errno != EEXIST))
				throw std::runtime_error(strerror(errno));
			if (found == std::string::npos || found == (data.size() - 1))
				return;
			index = found + 1;
		}
	}
...
void Path::cleanPath(std::string &path) const
{
	...
	// sanity check
	if (hasRoot && tokens.size() && (*tokens.begin()) == std::string(".."))
		throw std::runtime_error(std::string("Bad path: ") + data);
	// remove single dots
	tokens.erase(std::remove_if(tokens.begin(), tokens.end(), [](const std::string &s)
								{ return (s == std::string(".")); }),
				 tokens.end());
	// remove double dots
	auto itr = std::find_if(tokens.begin(), tokens.end(), [](const std::string &s)
							{ return (s != std::string("..")); });
	auto end = tokens.end();
	while (itr != end)
	{
...
std::string getProbName(const std::string &fileName, const std::vector<std::string> &exts)
{
	...
	{
		if (ends_with(probName, ext))
			probName.resize(probName.size() - ext.size());
	}
	return probName;
}
