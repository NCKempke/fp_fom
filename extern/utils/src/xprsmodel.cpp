...
{
	if (probSignalHandler)
		XPRSinterrupt(probSignalHandler, XPRS_STOP_CTRLC);
}
...
XPRSModel::XPRSModel()
{
	...
	if (status)
		throw std::runtime_error("Cannot initialize XPRESS library");
...
void XPRSModel::lpopt(char method, double tol, double gapTol)
{
	...
	case 's':
	{
		XPRS_CALL(XPRSlpoptimize, prob, "pdn");

		break;
	}
	case 'p':
	{
		XPRS_CALL(XPRSlpoptimize, prob, "p");

		break;
	}
	case 'd':
	{
		XPRS_CALL(XPRSlpoptimize, prob, "d");
	...
	default:
	{
		throw std::runtime_error("Unexpected method for XPRESS lpopt");
	}
	}

	/* Reset parameters to defaults. */
	intParam(IntParam::Crossover, -1);
	dblParam(DblParam::BarrierGap, 0.0);
}
...
std::vector<double> XPRSModel::postsolveSolution(const std::vector<double> &preX) const
{
	...
	int n = copy->ncols();
	FP_ASSERT(n == (int)preX.size());
	std::vector<char> xtype(n);
	copy->ctypes(&xtype[0]);
	for (int j = 0; j < n; j++)
	{
		if (xtype[j] != 'C')
			copy->fixCol(j, preX[j]);
	}
	copy->mipopt();
	int mipStatus = 0;
...
void XPRSModel::sol(double *x, int first, int last) const
{
	...
	int nglents = 0;
	int nsos = 0;
	XPRS_CALL(XPRSgetmipentities, prob, &nglents, &nsos, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr);
	...
	{
		if (isMIP)
			XPRS_CALL(XPRSgetmipsol, prob, x, nullptr);
		else
			XPRS_CALL(XPRSgetlpsol, prob, x, nullptr, nullptr, nullptr);
	}
	else
	{
		...
		if (isMIP)
			XPRS_CALL(XPRSgetmipsol, prob, &sol[0], nullptr);
		else
			XPRS_CALL(XPRSgetlpsol, prob, &sol[0], nullptr, nullptr, nullptr);
		std::copy(sol.begin() + first, sol.begin() + last + 1, x);
	}
}

void XPRSModel::dual_sol(double *y) const
{
	FP_ASSERT(prob);
	...
	XPRS_CALL(XPRSgetlpsol, prob, nullptr, nullptr, y, nullptr);
}

void XPRSModel::reduced_costs(double *redcosts) const
{
	FP_ASSERT(prob);
	...
	XPRS_CALL(XPRSgetlpsol, prob, nullptr, nullptr, nullptr, redcosts);
}
...
void XPRSModel::logging(bool log)
{
	...
	if (log)
	{
		XPRS_CALL(XPRSsetintcontrol, prob, XPRS_OUTPUTLOG, XPRS_OUTPUTLOG_FULL_OUTPUT);
		XPRS_CALL(XPRSaddcbmessage, prob, optimizermsg, nullptr, 0);
	}
	else
		XPRS_CALL(XPRSsetintcontrol, prob, XPRS_OUTPUTLOG, XPRS_OUTPUTLOG_NO_OUTPUT);
}
...
int XPRSModel::intParam(IntParam which) const
{
	...
	case IntParam::Crossover:
		XPRS_CALL(XPRSgetintcontrol, prob, XPRS_CROSSOVER, &value);
		break;
...
void XPRSModel::intParam(IntParam which, int value)
{
	...
	{
	case IntParam::Crossover:
		XPRS_CALL(XPRSsetintcontrol, prob, XPRS_CROSSOVER, value);
		break;
	case IntParam::Threads:
...
void XPRSModel::dblParam(DblParam which, double value)
{
	...
	case DblParam::TimeLimit:
		int xprmaxtime;
		if (value >= INT_MAX)
			xprmaxtime = 0; //< 0 means no time limit
		else
			xprmaxtime = (int)floor(value);
		XPRS_CALL(XPRSsetintcontrol, prob, XPRS_MAXTIME, -xprmaxtime);
		break;
	case DblParam::BarrierGap:
		XPRS_CALL(XPRSsetdblcontrol, prob, XPRS_BARGAPSTOP, value);
		break;
	case DblParam::FeasibilityTolerance:
...
// bkat
//  int XPRSModel::which(char solverparam)
//  {
//  	FP_ASSERT(prob);
...
// //bkat
// int XPRSModel::value(char solverparam)
// {
// 	FP_ASSERT(prob);
...
void XPRSModel::row(int ridx, SparseVector &row, char &sense, double &rhs, double &rngval) const
{
	FP_ASSERT(prob);
...
void XPRSModel::rows(SparseMatrix &matrix) const
{
	FP_ASSERT(prob);
	// get nnz
	int m = nrows();
...
void XPRSModel::col(int cidx, SparseVector &col, char &type, double &lb, double &ub, double &obj) const
{
	...
	if (status)
		type = 'C'; // cannot call XPRSgetcoltype on an LP
}

void XPRSModel::cols(SparseMatrix &matrix) const
{
	FP_ASSERT(prob);
	// get nnz
	int n = ncols();
...
void XPRSModel::rangeVal(double *rngval, int first, int last) const
{
	FP_ASSERT(prob);
	FP_ASSERT((first >= 0) && (first < nrows()));
	if (last == -1)
	...
	FP_ASSERT((last >= 0) && (last < nrows()));
	FP_ASSERT(first <= last);
	XPRS_CALL(XPRSgetrhsrange, prob, rngval, first, last);
}
...
XPRSModel *XPRSModel::presolvedmodel_impl()
{
	...
	else // return clone of current presolved problem
	{
		...
		for (int j = 0; j < n; j++)
		{
			if ((xtype[j] != 'C') && (xtype[j] != 'I') && (xtype[j] != 'B'))
